...
import { getAvailableSlotsResponseBody, getAvailableSlotsResponseBodyModel } from 'src/app/models/getavailableslots.response.body';
import { GetDoctorsResponseBodyModel ,GetDoctorsResponseBody} from 'src/app/models/getdoctors.response.body';
...
export class PatientComponent implements OnInit{
  ...
  getDoctors(){
    this.http.get<GetDoctorsResponseBody[]>(UsersService.url+UsersService.getDoctors).subscribe((data)=>{
      if(data != null){
        this.doctors= data
      }
    })
  ...
  getAvailableSlots(value: number){
    ...
    this.http.post<getAvailableSlotsResponseBody[]>(UsersService.url+UsersService.getAvailableSlots,body).subscribe((data)=>{
      if(data != null){
        this.slots= data
        console.log(data)
  ...
  reserve(){
    if(this.selectedAptId!=-1){
      ...
      this.http.patch<getAvailableSlotsResponseBody[]>(UsersService.url+UsersService.reserveApt,body).subscribe((data)=>{
        if(data != null){
  ...
  getPatientReservations(){
    ...
    this.http.patch<getReservationsResponseBody>(UsersService.url+UsersService.getReservations,body).subscribe((data)=>{
      if(data != null){
  ...
  cancelpt(rsr: number) {
    ...
    this.http.patch(UsersService.url+UsersService.cancelReservations,body).subscribe((data)=>{
      if(data != null){
  ...
  applyChange(){
    ...
    if(this.selectedAptId != -1){
      ...
      this.http.patch(UsersService.url+UsersService.updateReservation,body).subscribe((data)=>{
        if(data != null){
